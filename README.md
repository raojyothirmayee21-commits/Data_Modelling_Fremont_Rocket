...
Honest note on right-sizing: for the current data volume alone, ADF → Azure SQL → Power BI with no Databricks and no lake would be sufficient and cheaper. The medallion design here is the target state. See Phased rollout for how I'd actually sequence it.
